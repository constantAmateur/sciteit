...
class SubsciteitExists(Exception): pass

class Subsciteit(Thing, Printable):
    ...
    _defaults = dict(static_path = g.static_path,
                     ...
                     over_18 = False,
                     mod_actions = 0,
                     sponsorship_text = "this sciteit is sponsored by",
    ...
    # note: for purposely unrenderable sciteits (like promos) set author_id = -1
    @classmethod
    def _new(cls, name, title, author_id, ip, lang = g.lang, type = 'public',
        ...
        with g.make_lock('create_sr_' + name.lower()):
            try:
                ...
                raise SubsciteitExists
            except NotFound:
                if "allow_top" not in kw:
                    kw['allow_top'] = True
                sr = Subsciteit(name = name,
                               title = title,
                               lang = lang,
                               type = type,
                               over_18 = over_18,
                ...
                #clear cache
                Subsciteit._by_name(name, _update = True)
                return sr
    ...
    def _by_name(cls, names, stale=False, _update = False):
        ...
        if to_fetch:
            ...
            srs = {}
            srids = sgm(g.cache, to_fetch.keys(), _fetch, prefix='subsciteit.byname', stale=stale)
            if srids:
        ...
        if ret and single:
            ...
            raise NotFound, 'Subsciteit %s' % name
        else:
            return ret

    @classmethod
    @memoize('subsciteit._by_domain')
    ...
    @classmethod
    def load_subsciteits(cls, links, return_dict = True, stale=False):
        """returns the subsciteits for a list of links. it also preloads the
        permissions for the current user."""
        srids = set(l.sr_id for l in links
                    if getattr(l, "sr_id", None) is not None)
        subsciteits = {}
        if srids:
            subsciteits = cls._byID(srids, data=True, stale=stale)

        if subsciteits and c.user_is_loggedin:
            # dict( {Subsciteit,Account,name} -> Relationship )
            SRMember._fast_query(subsciteits.values(), (c.user,),
                                 ('subscriber','contributor','moderator'),
                                 data=True, eager_load=True, thing_data=True)

        return subsciteits if return_dict else subsciteits.values()

    #rising uses this to know which subsciteits to include, doesn't
    #work for all/friends atm
    def rising_srs(self):
        if c.default_sr or not hasattr(self, '_id'):
            user = c.user if c.user_is_loggedin else None
            sr_ids = self.user_subsciteits(user)
        else:
            sr_ids = (self._id,)
        return sr_ids

    def get_links(self, sort, time,**kw):
        from r2.lib.db import queries
        return queries.get_links(self, sort, time,**kw)
    ...
    def add_props(cls, user, wrapped):
        ...
        defaults = Subsciteit.default_subsciteits()
        for item in wrapped:
            ...
            if (g.lounge_sciteit and item.name == g.lounge_sciteit
    ...
    def top_lang_srs(cls, lang, limit, filter_allow_top = False, over18 = True,
        ...
        sr_ids = sr_pops.pop_sciteits(lang, over18, over18_only, filter_allow_top = filter_allow_top)
        sr_ids = sr_ids[:limit]

        return (sr_ids if ids
                else Subsciteit._byID(sr_ids, data=True, return_dict=False, stale=stale))

    @classmethod
    def default_subsciteits(cls, ids = True, over18 = False, limit = g.num_default_sciteits,
                           stale=True):
        """
        Generates a list of the subsciteits any user with the current
        set of language preferences and no subscriptions would see.

        An optional kw argument 'limit' is defaulted to g.num_default_sciteits
        """

        # we'll let these be unordered for now
        auto_srs = []
        if g.automatic_sciteits:
            auto_srs = map(lambda sr: sr._id,
                           Subsciteit._by_name(g.automatic_sciteits, stale=stale).values())
        ...
        return rv if ids else Subsciteit._byID(rv, data=True, return_dict=False, stale=stale)

    @classmethod
    @memoize('random_sciteits', time = 1800)
    def random_sciteits(cls, user_name, sr_ids, limit):
        """This gets called when a user is subscribed to more than 50
        sciteits. Randomly choose 50 of those sciteits and cache it for
        a while so their front page doesn't jump around."""
        return random.sample(sr_ids, limit)

    @classmethod
    def random_sciteit(cls, limit = 1000, over18 = False):
        srs = cls.top_lang_srs(c.content_langs, limit,
                               filter_allow_top = False,
                               over18 = over18,
                               over18_only = over18,
                               ids=True)
        return (Subsciteit._byID(random.choice(srs))
                if srs else Subsciteit._by_name(g.default_sr))

    @classmethod
    def user_subsciteits(cls, user, ids=True, over18=False, limit=DEFAULT_LIMIT,
                        stale=False):
        """
        subsciteits that appear in a user's listings. If the user has
        subscribed, returns the stored set of subscriptions.
        
        limit - if it's Subsciteit.DEFAULT_LIMIT, limits to 50 subs
                (100 for gold users)
        ...
        if limit is Subsciteit.DEFAULT_LIMIT:
            if user and user.gold:
                # Goldies get extra subsciteits
                limit = Subsciteit.gold_limit
            else:
        ...
        if user and user.has_subscribed:
            sr_ids = Subsciteit.reverse_subscriber_ids(user)

            if limit and len(sr_ids) > limit:
                sr_ids.sort()
                sr_ids = cls.random_sciteits(user.name, sr_ids, limit)
            return sr_ids if ids else Subsciteit._byID(sr_ids,
                                                      data=True,
                                                      return_dict=False,
                                                      stale=stale)
        else:
            return cls.default_subsciteits(ids = ids, over18=over18,
                                          limit=g.num_default_sciteits,
                                          stale=stale)

    @classmethod
    @memoize('subsciteit.special_sciteits')
    def special_sciteits_cache(cls, user_id, query_param):
        sciteits = SRMember._query(SRMember.c._name == query_param,
        ...
        return [ sr._thing1_id for sr in sciteits ]

    # Used to pull all of the SRs a given user moderates or is a contributor
    # to (which one is controlled by query_param)
    @classmethod
    def special_sciteits(cls, user, query_param, _update=False):
        return cls.special_sciteits_cache(user._id, query_param, _update=_update)

    def is_subscriber_defaults(self, user):
        if user.has_subscribed:
            return self.is_subscriber(user)
        else:
            return self in self.default_subsciteits(ids = False)

    @classmethod
    def subscribe_defaults(cls, user):
        if not user.has_subscribed:
            for sr in cls.user_subsciteits(None, False,
                                          limit = g.num_default_sciteits):
    ...
    def submit_sr_names(cls, user):
        """subsciteit names that appear in a user's submit page. basically a
        sorted/rearranged version of user_subsciteits()."""
        srs = cls.user_subsciteits(user, ids = False)
    ...
    def get_images(self):
        ...
        uploaded for custom styling of this subsciteit. 
        """
    ...
    def add_image(self, name, url, max_num = None):
        """
        Adds an image to the subsciteit's image list.  The resulting
        ...
        The Subsciteit will be _dirty if a new image has been added to
    ...
    def del_image(self, name):
        ...
        The Subsciteit will be _dirty if image has been removed from
        its images list, and no _commit is called.
        """
    ...
    def __eq__(self, other):
        ...
        if isinstance(self, FakeSubsciteit):
...
class FakeSubsciteit(Subsciteit):
    over_18 = False
    _nodb = True

    def __init__(self):
        Subsciteit.__init__(self)
        self.title = ''
...
class FriendsSR(FakeSubsciteit):
    name = 'friends'
    title = 'friends'
    ...
    def get_links(self, sort, time,**kw):
        ...
        if g.use_query_cache:
            # with the precomputer enabled, this Subsciteit only supports
...
class _DefaultSR(FakeSubsciteit):
...
# This is the base class for the instantiated front page sciteit
class DefaultSR(_DefaultSR):
    def __init__(self):
        _DefaultSR.__init__(self)
        try:
            self._base = Subsciteit._by_name(g.default_sr, stale=True)
        except NotFound:
            self._base = None
...
class MultiSciteit(_DefaultSR):
    ...
    def __init__(self, sr_ids, path):
        ...
        srs = Subsciteit._byID(self.sr_ids, return_dict=False)
        self.banned_sr_ids = []
        self.kept_sr_ids = []
        for sr in srs:
            if sr._spam:
    ...
    def get_all_comments(self):
        ...
        srs = Subsciteit._byID(self.kept_sr_ids, return_dict=False)
        results = [get_sr_comments(sr) for sr in srs]
        return merge_results(*results)

class RandomSciteit(FakeSubsciteit):
    name = 'random'
    header = ""

class RandomNSFWSciteit(FakeSubsciteit):
    name = 'randnsfw'
    header = ""
...
class ModContribSR(_DefaultSR):
    ...
    def sr_ids(self):
        if c.user_is_loggedin:
            return Subsciteit.special_sciteits(c.user, self.query_param)
        else:
            return []
...
class SubSR(FakeSubsciteit):
    stylesheet = 'subsciteit.css'
...
class DomainSR(FakeSubsciteit):
    @property
    def path(self):
        return '/domain/' + self.domain

    def __init__(self, domain):
        FakeSubsciteit.__init__(self)
        self.domain = domain
        self.name = domain 
        self.title = domain + ' ' + _('on sciteit.com')
...
Contrib = ContribSR()
All = AllSR()
Random = RandomSciteit()
RandomNSFW = RandomNSFWSciteit()

Subsciteit._specials.update(dict(friends = Friends,
                                randnsfw = RandomNSFW,
                                random = Random,
                                mod = Mod,
                                contrib = Contrib,
...
class SRMember(Relation(Subsciteit, Account)): pass
Subsciteit.__bases__ += (UserRel('moderator', SRMember),
                        UserRel('contributor', SRMember),
                        UserRel('subscriber', SRMember, disable_ids_fn = True),
                        UserRel('banned', SRMember))

class SubsciteitPopularityByLanguage(tdb_cassandra.View):
    _use_db = True
    _value_type = 'pickle'
    _use_new_ring = True
