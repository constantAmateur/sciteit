function open_menu(menu) {
    //alert($(menu).parent().html());
    $(menu).next(".drop-choices").not(".inuse")
    ...
    try {
        if(statusfunc == null)
            statusfunc = function(x) { 
                return sciteit.status_msg.submitting; 
            };
...
function form_error(form) {
    return function(r) {
    	alert("OH NO! Something went wrong when saving data to db...");
...
function post_pseudo_form(form, where, block) {
    ...
    $(form).find(".error").not(".status").hide();
    $(form).find(".status").html(sciteit.status_msg.submitting).show();
...
function subscribe(sciteit_name) {
    return function() { 
        if (sciteit.logged) {
            $.things(sciteit_name).find(".entry").addClass("likes");
            $.request("subscribe", {sr: sciteit_name, action: "sub"});
        }
    };
};

function unsubscribe(sciteit_name) {
    return function() { 
        if (sciteit.logged) {
            $.things(sciteit_name).find(".entry").removeClass("likes");
            $.request("subscribe", {sr: sciteit_name, action: "unsub"});
        }
    };
};
...
function friend(user_name, container_name, type) {
    return function() {
        if (sciteit.logged) {
            encoded = encodeURIComponent(sciteit.referer);
...
function morechildren(form, link_id, children, depth, pv_hex) {
    $(form).html(sciteit.status_msg.loading)
        .css("color", "red");
...
function moremessages(elem) {
    $(elem).html(sciteit.status_msg.loading).css("color", "red");
...
function fetch_title() {
    ...
    var url = $("#url").val();
    if (url) {
        status.show().text(sciteit.status_msg.loading);
        error.hide();
...
/**** sr completing ****/
function sr_cache() {
    if (!$.defined(sciteit.sr_cache)) {
        sciteit.sr_cache = new Array();
    }
    return sciteit.sr_cache;
}

function highlight_sciteit(item) {
...
function sr_search(query) {
    ...
    var cache = sr_cache();
    if (!cache[query]) {
        $.request('search_sciteit_names', {query: query},
                  function (r) {
                      cache[query] = r['names'];
...
function sr_name_up(e) {
    ...
    else if (e.keyCode == 27 && sciteit.orig_sr) {
        $("#sr-autocomplete").val(sciteit.orig_sr);
        hide_sr_name_list();
    }
    else if (new_sr_name != old_sr_name) {
        sciteit.orig_sr = new_sr_name;
...
function sr_name_down(e) {
    ...
    if (e.keyCode == 38 || e.keyCode == 40) {
        ...
        highlight_sciteit(new_row);
        if (new_row) {
            input.val($.trim(new_row.text()));
        }
        else {
            input.val(sciteit.orig_sr);
        }
        return false;
    }
...
function sr_dropdown_mdown(row) {
    sciteit.sr_mouse_row = row; //global
    return false;
}

function sr_dropdown_mup(row) {
    if (sciteit.sr_mouse_row == row) {
        var name = $(row).text();
...
function expando_cache() {
    if (!$.defined(sciteit.thing_child_cache)) {
        sciteit.thing_child_cache = new Array();
    }
    return sciteit.thing_child_cache;
}
...
function toggle_nominate_span(elem) {
  var form = $(elem).parents("form")[0];
  $(form).children().toggle('slow');
}

function set_nominate(elem, value) {
  change_state(elem, "nominate/" + value);
  $(elem).children().toggle();
}

function toggle_bestresponse_span(elem) {
  var form = $(elem).parents("form")[0];
  $(form).children().toggle('slow');
}

function set_bestresponse(elem, value) {
  change_state(elem, "bestresponse/" + value);
  $(elem).children().toggle();
}
...
var toolbar_p = function(expanded_size, collapsed_size) {
    /* namespace for functions related to the sciteit toolbar frame */
    ...
    this.panel_loadurl = function(url) {
        try {
            var cur = window.parent.inner_toolbar.sciteit_panel.location;
            if (cur == url) {
                return false;
            } else {
                if (pop_obj != null) {
...
function fetch_parent(elem, parent_permalink, parent_id) {
    $(elem).css("color", "red").html(sciteit.status_msg.loading);
    var thing = $(elem).thing();
...
$(function() {
        ...
        if (sciteit.logged) {
            $.cookie_name_prefix(sciteit.logged);
        }
        else {
            //populate_click_gadget();
        }
        /* set up the cookie domain */
        $.default_cookie_domain(sciteit.cur_domain.split(':')[0]);
...
function parse_domain(url) {
    ...
    if (!domain) {
        ...
        var sciteit = url.match(/\/r\/([^/]+)/)
        if (sciteit) {
            domain = "self." + sciteit[1].toLowerCase();
        }
    }
    return domain;
}
