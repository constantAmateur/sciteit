/* The sciteit extension for jquery.  This file is intended to store
...
$.debug = function(message) {
    if ($.with_default(sciteit.debug, false)) {
        return $.log(message);
    }
}
$.fn.debug = function() { 
...
/* customized requests (formerly sciteitRequest) */
...
$.request = function(op, parameters, worker_in, block, type, 
    ...
    /* 
       Uniquitous sciteit AJAX poster.  Automatically addes
       handleResponse(action) worker to deal with the API result.  The
       current subsciteit (sciteit.post_site), the user's modhash
       (sciteit.modhash) and whether or not we are in a frame
       (sciteit.cnameframe) are also automatically sent across.
     */
    var action = op;
    ...
    if (rate_limit(op) || (window != window.top && !sciteit.cnameframe && !sciteit.external_frame))
        return;
    ...
    /* set the subsciteit name if there is one */
    if (sciteit.post_site) 
        parameters.r = sciteit.post_site;

    /* flag whether or not we are on a cname */
    if (sciteit.cnameframe) 
        parameters.cnameframe = 1;

    /* add the modhash if the user is logged in */
    if (sciteit.logged) 
        parameters.uh = sciteit.modhash;

    parameters.renderstyle = sciteit.renderstyle;

    if(have_lock) {
        op = api_loc + op;
        /*if( document.location.host == sciteit.ajax_domain ) 
            /* normal AJAX post */
        ...
        /*else { /* cross domain it is... * /
            op = "http://" + sciteit.ajax_domain + op + "?callback=?";
...
rate_limit = function() {
    ...
    var rate_limits = {"vote": 333, "comment": 5000, "criticism":5000,
...
$.fn.vote = function(vh, callback, event, ui_only) {
    ...
    if (sciteit.logged && $(this).hasClass("arrow")) {
...
$.fn.thing_id = function(class_filter) {
    ...
    /* Returns the (sciteit) ID of the current element's thing */
...
/* old sciteit-specific cookie functions */
