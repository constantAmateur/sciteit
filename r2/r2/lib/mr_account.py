...
time psql -F"\t" -A -t -d newsciteit -U $USER -h $LINKDBHOST \
     -c "\\copy (select d.value, d.thing_id, 'data', '$KIND',
                        d.key
                   from sciteit_data_$KIND d, sciteit_thing_$KIND t
...
from r2.models import Account, Subsciteit, Link, Comment, NotFound
