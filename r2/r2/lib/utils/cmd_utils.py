...
def subs_contribs(sr_name = 'betateam'):
    """Convert all subscribers of a given subsciteit to
       contributors. Useful for forming opt-in beta teams"""
    from r2.models import Subsciteit, SRMember

    sr = Subsciteit._by_name(sr_name)
    q = SRMember._query(SRMember.c._thing1_id == sr._id)

    for rel in rels:
        if rel._name == 'subscriber':
            sr.add_contributor(rel._thing2)
            Subsciteit.special_sciteits(rel._thing2, 'contributor', _update=True)
