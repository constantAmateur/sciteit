"""
Generate the data for the listings for the time-based Subsciteit
...
cd ~/sciteit/r2
time psql -F"\t" -A -t -d newsciteit -U $USER -h $LINKDBHOST \
     -c "\\copy (select t.thing_id, 'thing', 'link',
                        t.ups, t.downs, t.deleted, t.spam, extract(epoch from t.date)
                   from sciteit_thing_link t
                  where not t.spam and not t.deleted
                     and t.date > now() - interval '1 year'
                  )
                  to 'sciteit_thing_link.dump'"
time psql -F"\t" -A -t -d newsciteit -U $USER -h $LINKDBHOST \
     -c "\\copy (select d.thing_id, 'data', 'link',
                        d.key, d.value
                   from sciteit_data_link d, sciteit_thing_link t
                  where t.thing_id = d.thing_id
                    and not t.spam and not t.deleted
                    and (d.key = 'url' or d.key = 'sr_id')
                    and t.date > now() - interval '1 year'
                  )
                  to 'sciteit_data_link.dump'"
cat sciteit_data_link.dump sciteit_thing_link.dump | sort -T. -S200m | paster --plugin=r2 run $INI r2/lib/mr_top.py -c "join_links()" > links.joined
...
"""
## """
## psql -F"\t" -A -t -d newsciteit -U ri -h $LINKDBHOST \
...
# allotted time (e.g. the year listing in a subsciteit that hasn't had
# a submission in the last year), we won't write out an empty
# list. I'll call it a feature.

import sys

from r2.models import Account, Subsciteit, Link
