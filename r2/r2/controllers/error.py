...
try:
    ...
    from sciteit_base import SciteitController, Cookies
    from r2.models.subsciteit import DefaultSR, Subsciteit
...
'''<html>
  <head>
    <title>sciteit broke!</title>
  </head>
  <body>
    <div style="margin: auto; text-align: center">
      <p>
        <a href="/">
          <img border="0" src="%s" alt="you broke sciteit" />
        </a>
      </p>
      <p>
        %s
...
class ErrorController(SciteitController):
    ...
    # List of admins to blame (skip the first admin, "sciteit")
    ...
    def send403(self):
        ...
        c.site = DefaultSR()
        res = pages.SciteitError(_("forbidden (%(domain)s)") %
    ...
    def send404(self):
        ...
        if c.site.spammy() and not c.user_is_admin:
            ...
            if "message" in ban_info:
                ...
                subject = ("the subsciteit /r/%s has been incorrectly banned" %
                           c.site.name)
                lnk = ("/r/sciteitrequest/submit?url=%s&title=%s"
                       % (url_escape("http://%s/r/%s" % (g.domain, c.site.name)),
                          ("the subsciteit /r/%s has been incorrectly banned" %
                           c.site.name)))
                message = strings.banned_subsciteit % dict(link = lnk)

            res = pages.SciteitError(_('this sciteit has been banned'),
                                    unsafe(safemarkdown(message)))
            return res.render()
        else:
            return pages.Sciteit404().render()
...
def handle_awful_failure(fail_text):
    ...
    try:
        ...
        return sciteitbroke % (rand.randint(1,NUM_FAILIENS), fail_text)
    except:
