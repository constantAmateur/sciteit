...
from r2.models.subsciteit import Subsciteit
...
cache_affecting_cookies = ('sciteit_first','over18','_options')
...
def firsttime():
    if (request.user_agent and
        ...
        not get_sciteitfirst('mobile_suggest')):
        set_sciteitfirst('mobile_suggest','first')
        return 'mobile_suggest'
    elif get_sciteitfirst('firsttime'):
        return False
    else:
        set_sciteitfirst('firsttime','first')
        return True

def get_sciteitfirst(key,default=None):
    try:
        val = c.cookies['sciteit_first'].value
...
def set_sciteitfirst(key,val):
    try:
        cookie = simplejson.loads(c.cookies['sciteit_first'].value)
        cookie[key] = val
    except (ValueError,TypeError,KeyError),e:
        # invalid JSON data; we'll just construct a new cookie
        cookie = {key: val}

    c.cookies['sciteit_first'] = Cookie(simplejson.dumps(cookie),
                                       expires = NEVER)
...
def organic_pos():
    ...
    pos = get_sciteitfirst(organic_pos_key, 0)
...
def set_organic_pos(pos):
    ...
    set_sciteitfirst(organic_pos_key, pos)
...
def set_content_type():
    ...
    if e.has_key('extension'):
        ...
        if ext in ("mobile", "m") and not request.GET.get("keep_extension"):
            try:
                if request.cookies['sciteit_mobility'] == "compact":
                    c.extension = "compact"
        ...
        if ext in ("mobile", "m", "compact"):
            if request.GET.get("keep_extension"):
                c.cookies['sciteit_mobility'] = Cookie(ext, expires = NEVER)
...
def set_host_lang():
    ...
    host_lang = request.environ.get('sciteit-prefer-lang')
    if host_lang:
...
def set_cnameframe():
    if (bool(request.params.get(utils.UrlParser.cname_get)) 
        ...
        c.cname = True
        request.environ['SCITEIT_CNAME'] = 1
...
class MinimalController(BaseController):
    ...
    def pre(self):
        ...
        c.domain_prefix = request.environ.get("sciteit-domain-prefix",
                                              g.domain_prefix)
...
class SciteitController(MinimalController):
